  ...
  if (!listings || listings.length === 0) {
    return (
      <div className={`rounded-xl shadow-sm p-6 h-full ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        <div className={`flex items-center justify-center h-48 border border-dashed rounded-lg ${isDark ? 'border-gray-700' : 'border-gray-200'
          }`}>
    ...
    <div className={`rounded-xl shadow-sm p-6 h-full ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
      ...
      <div className="space-y-4">
        {displayedListings.map((listing) => (
          <div
            key={listing.id}
            className={`flex items-start p-3 rounded-lg transition-colors ${isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-50'
              }`}
          >
            ...
            <div className="ml-3 flex-1 min-w-0">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ml-2 ${listing.status === 'Active'
