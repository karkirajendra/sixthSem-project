...
import { FaPaperPlane, FaUser, FaComments, FaHome, FaUsers, FaStore, FaCrown, FaBuilding } from 'react-icons/fa';
...
const SellerChat = () => {
  ...
  const [activeTab, setActiveTab] = useState('all'); // 'all' or 'properties'
  ...
  const getMessageInfo = (msg) => {
    ...
    return {
      ...
      roleLabelColor: isAdmin ? 'text-purple-600' : (isBuyer ? 'text-blue-600' : 'text-green-600')
    };
  };
  ...
  const getRoleDisplayInfo = (user) => {
    ...
    return {
      icon: isAdmin ? <FaCrown className="text-purple-500" /> : (isBuyer ? <FaUser className="text-blue-500" /> : <FaStore className="text-green-500" />),
      bgColor: isAdmin ? 'bg-purple-100' : (isBuyer ? 'bg-blue-100' : 'bg-green-100'),
      textColor: isAdmin ? 'text-purple-600' : (isBuyer ? 'text-blue-600' : 'text-green-600'),
      roleLabel: isAdmin ? 'Admin' : (isBuyer ? 'Buyer' : 'Seller'),
      badgeColor: isAdmin ? 'bg-purple-100 text-purple-600' : (isBuyer ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600')
    };
  };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Seller Chat Center</h1>
                ...
                <div className="flex items-center">
                  <div className="relative">
                    {getOtherParticipant(selectedRoom).profile?.avatar ? (
                      ...
                      <div className={`w-12 h-12 rounded-full ${getRoleDisplayInfo(getOtherParticipant(selectedRoom)).bgColor} flex items-center justify-center mr-4`}>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center flex-wrap gap-2">
                      <span className={`text-sm px-2 py-1 rounded-full ${getRoleDisplayInfo(getOtherParticipant(selectedRoom)).badgeColor}`}>
                    ...
                    return (
                      <div key={msg._id} className={`flex ${messageInfo.alignment}`}>
                        <div className="max-w-xs lg:max-w-md">
                          ...
                          {/* Message bubble */}
                          <div className={`px-4 py-3 rounded-lg shadow-sm border ${messageInfo.bubbleColor} ${
                            messageInfo.isCurrentUser 
                              ? 'rounded-tr-none' 
                              : 'rounded-tl-none'
                          }`}>
                ...
                <div className="flex items-center">
                  <div className="flex-1 relative">
                    <input
                      ...
                      placeholder="Type your seller message..."
                      className="w-full px-4 py-3 border border-gray-300 rounded-l-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"
                      maxLength={1000}
                    />
                  </div>
                  <button
                    type="submit"
                    className="bg-gradient-to-r from-green-500 to-green-600 text-white px-6 py-3 rounded-r-lg hover:from-green-600 hover:to-green-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-gray-500 bg-gray-50">
              <div className="text-center">
                <div className="bg-green-100 rounded-full p-6 mx-auto mb-4 w-24 h-24 flex items-center justify-center">
                  <FaComments className="text-4xl text-green-500" />
                </div>
